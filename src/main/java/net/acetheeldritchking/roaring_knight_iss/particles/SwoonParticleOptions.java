...
public class SwoonParticleOptions implements ParticleOptions {
    ...
    public SwoonParticleOptions(float xf, float yf, float zf, boolean mirror, boolean vertical, float scale) {
        this.scale = scale;
        this.xf = xf;
        this.yf = yf;
        this.zf = zf;
    ...
    public static StreamCodec<? super ByteBuf, SwoonParticleOptions> STREAM_CODEC = StreamCodec.of(
            (buf, option) ->
            {
            ...
            (byteBuf -> new SwoonParticleOptions(byteBuf.readFloat(), byteBuf.readFloat(), byteBuf.readFloat(), byteBuf.readBoolean(), byteBuf.readBoolean(), byteBuf.readFloat()))
    );

    public static MapCodec<SwoonParticleOptions> MAP_CODEC = RecordCodecBuilder.mapCodec(object ->
            object.group(
            ...
            ).apply(object, SwoonParticleOptions::new)
    );

    @Override
    public ParticleType<?> getType() {
        return RKParticleRegistry.SWOON_PARTICLE.get();
    }
}
